...
<body class="bg-blue-100 min-h-screen flex items-center justify-center p-6">
    <div class="bg-white rounded-2xl shadow-lg w-full max-w-3xl p-10 space-y-6">
        ...
        <%
            ...
            if (professorDAO.consultar_por_id(professor) != null) {
        %>
        <form method="post" action="../../components/operations/atualizar_dados_prof.jsp" class="grid grid-cols-1 md:grid-cols-2 gap-6">

            <div>
                <label class="block text-sm font-semibold text-blue-800">Código</label>
                <input type="text" name="id" value="<%= professor.getId()%>" readonly
                       class="mt-1 w-full border border-blue-300 bg-gray-100 rounded-lg p-3 text-gray-600" />
            </div>

            <div>
                <label class="block text-sm font-semibold text-blue-800">Nome</label>
            ...
            <div>
                ...
                <input type="text" name="email" value="<%= professor.getEmail()%>" required
                       class="mt-1 w-full border border-blue-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500" />
            </div>
            ...
            <div class="relative">
                ...
                <input type="password" id="senha" name="senha" value="<%= professor.getSenha()%>" required
            ...
            <div>
                ...
                <input type="text" name="telefone" value="<%= professor.getTelefone()%>" required
                       class="mt-1 w-full border border-blue-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500" />
            </div>

            <div>
                <label class="block text-sm font-semibold text-blue-800">CPF</label>
                <input type="text" name="cpf" value="<%= professor.getCpf()%>" required
                       class="mt-1 w-full border border-blue-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500" />
            </div>

            <input type="hidden" name="matricula" value="<%= professor.getId()%>" />
